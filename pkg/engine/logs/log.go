...
func init() {
	core.Register(core.MiddleLevel, "日志", InitLogs)
}
...
func InitLogs() (fn func(), err error) {
	fn = func() {}
	logger := NewLog(
		WithLogPath(config.GlobConfig.Log.LogDir),
		WithStorageDays(config.GlobConfig.Log.StorageDays),
	)
	Logger, err = logger.InitLog()
	if err != nil {
		return
	}
	log.Println(fmt.Sprintf("日志组件初始化成功！日志所在目录：%v，保存天数为：%v", config.GlobConfig.Log.LogDir, config.GlobConfig.Log.StorageDays))
	return
}
