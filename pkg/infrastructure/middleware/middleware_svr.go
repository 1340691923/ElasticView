...
func NewMiddleWareService(cfg *config.Config, orm *sqlstore.SqlStore, log *logger.AppLogger, jwtSvr *jwt_svr.Jwt, res *response.Response, gmUserService *gm_user.GmUserService, rbac *access_control.Rbac, routerEngine *web_engine.WebEngine, pluginRegistry manager.Service) *MiddleWareService {
	return &MiddleWareService{cfg: cfg, orm: orm, log: log, jwtSvr: jwtSvr, res: res, gmUserService: gmUserService, rbac: rbac, routerEngine: routerEngine, pluginRegistry: pluginRegistry}
}
...
func (this *MiddleWareService) CheckVersion(c *gin.Context) {
	if !this.cfg.DeBug && config.Version != c.GetHeader("X-Version") && c.GetHeader("X-Version") != "test" {
		err := my_error.NewError(fmt.Sprintf("后台已更新版本（新版本：%s,您的版本：%s），请刷新页面", config.Version, c.GetHeader("X-Version")), ERROR_CEHCK_VERSION_FAIL)
		this.res.Error(c, err)
		c.Abort()
		return
	}
