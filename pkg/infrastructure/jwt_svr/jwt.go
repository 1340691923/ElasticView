...
func NewJwt(cfg *config.Config, log *logger.AppLogger, orm *orm.Gorm) *Jwt {
	return &Jwt{cfg: cfg, log: log, orm: orm}
}
...
func (this *Jwt) CreateToken(gmUser model.GmUserModel) (string, error) {
	...
	*/
	...
	token, err := tokenClaims.SignedString(util.Str2bytes(this.GetJwtKey())) // SignedString  该方法内部生成签名字符串，再用于获取完整、已签名的token
	if err != nil {
...
func (this *Jwt) ParseToken(token string) (*Claims, error) {
	tokenClaims, err := jwt.ParseWithClaims(token, &Claims{}, func(token *jwt.Token) (interface{}, error) { //ParseWithClaims   用于解析鉴权的声明，方法内部主要是具体的解码和校验的过程，最终返回*Token
		return util.Str2bytes(this.GetJwtKey()), nil
	})
...
func (this *Jwt) GetJwtKey() string {
	data := &model.JwtKeyModel{}
	this.orm.First(&data)

	if data.JwtKey == "" {
		data.JwtKey = util.GetUUid()
