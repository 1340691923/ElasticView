...
func NewGmUserService(log *logger.AppLogger, gmUserDao *dao.GmUserDao, gmRoleDao *dao.GmRoleDao, jwt *jwt_svr.Jwt, pluginRegistry manager.Service, cfg *config.Config, orm *orm.Gorm, oAuthServiceRegistry *oauth.OAuthServiceRegistry) *GmUserService {
...
func (this *GmUserService) CheckLogin(ctx context.Context, username, password string) (token string, err error) {
	...
	this.gmUserDao.UpdateLastLoginTime(ctx, gmUser.Id)
	...
	for _, srcRoute := range srcRoutes {
		destRoutes.Children = append(destRoutes.Children, &Route{
			...
			Meta: &RouteMeta{
				...
				Service: srcRoute.Meta.Service,
				Hidden:  srcRoute.Meta.Hidden,
			},
		})
...
func addRoutes2C(destRoutes *Route, srcRoutes []*build.Route) {
	for _, srcRoute := range srcRoutes {
		...
		if len(srcRoute.Children) > 0 {
			...
			if !srcRoute.Meta.Service && srcRoute.Meta.Hidden {
				destRoutes.Children = append(destRoutes.Children, &Route{
					Path: srcRoute.Path,
					Name: util.StringPtr(srcRoute.Path),
					Meta: &RouteMeta{
...
func (this *GmUserService) GetRoleList2C(ctx context.Context) (routes []*Route, qiankunMicroApps []*QiankunMicroApp) {
	...
	for _, plugin := range this.pluginRegistry.Plugins(ctx) {
		...
		addRoutes2C(pluginRoutes, frontendRoutes)
