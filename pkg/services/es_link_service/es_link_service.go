...
func NewEsLinkService(orm *orm.Gorm, logger *logger.AppLogger, esClientService *es.EsClientService, eslinkCfgV2Dao *dao.EslinkCfgV2Dao, gmRoleEslinkCfgV2Dao *dao.GmRoleEslinkCfgV2Dao, eslinkRoleCfgReletionDao *dao.EslinkRoleCfgReletionDao, gmUserDao *dao.GmUserDao, esLinkV2Dao *dao.EsLinkV2Dao) *EsLinkService {
...
// 获取列表信息
func (this *EsLinkService) GetListAction(ctx context.Context, userID int, roleID []int, page, pageSize int) (esLinkList []*vo.EsLink, count int64, err error) {
	...
	err = builder.Select("count(*)").Scan(&count).Error
	if err != nil {
	...
	err = builder.Select("*").Limit(pageSize).Offset(orm.CreatePage(page, pageSize)).Scan(&esLinks).Error
	if err != nil {
		err = errors.WithStack(err)
		return
	}
...
where es_link_id = ? `, esLink.Id).Scan(&esCfgs).Error
	...
	return esLinkList, count, nil
}

func (this *EsLinkService) GetEsCfgList(ctx context.Context, userId int, page, pageSize int) (res []*vo.EsLinkConfigV2, count int, err error) {
	var esCfgs []model.EslinkCfgV2
	esCfgs, count, err = this.eslinkCfgV2Dao.SelectByCreateBy(ctx, userId, page, pageSize)
	if err != nil {
		return nil, 0, errors.WithStack(err)
	}
...
func (this *EsLinkService) GetEsCfgOpt(ctx context.Context, userId int) (res []*vo.EsLinkConfigOpt, err error) {
	...
	esCfgs, _, err = this.eslinkCfgV2Dao.SelectByCreateBy(ctx, userId, 1, 10000)
