...
func NewManagerUserController(baseController *BaseController, log *logger.AppLogger, cfg *config.Config, orm *sqlstore.SqlStore, jwtSvr *jwt_svr.Jwt, gmUserService *gm_user.GmUserService, routerEngine *web_engine.WebEngine, evUpdate *updatechecker.EvUpdate, pluginRegistry manager.Service) *ManagerUserController {
...
func (this *ManagerUserController) UserListAction(ctx *gin.Context) {
	...
	list, err = this.gmUserService.Select(ctx, this.gmUserService.IsAdminUser(this.GetRoleCache(ctx)))
	...
	this.Success(ctx, response.SearchSuccess, users)
	return
}
