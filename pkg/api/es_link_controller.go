...
func NewEsLinkController(baseController *BaseController, log *logger.AppLogger, esClientService *es.EsClientService, sqlx *sqlstore.SqlStore, esLinkService *es_link_service.EsLinkService, jwtSvr *jwt_svr.Jwt, esCache *cache_service.EsCache) *EsLinkController {
...
func (this *EsLinkController) ListAction(ctx *gin.Context) {
	...
	list, err := this.esLinkService.GetListAction(ctx, userInfo.UserID, this.GetRoleCache(ctx))
	if err != nil {
		this.Error(ctx, err)
		return
	}
...
func (this *EsLinkController) GetEsCfgList(ctx *gin.Context) {
	...
	list, err := this.esLinkService.GetEsCfgList(ctx, userInfo.UserID)
	if err != nil {
		this.Error(ctx, err)
		return
	}
