...
func NewPluginController(baseController *BaseController, log *logger.AppLogger, orm *sqlstore.SqlStore, pluginService *plugin_service.PluginService, eveService *eve_service.EvEService, pluginInstaller *plugin_install_service.PluginInstaller) *PluginController {
...
func (this *PluginController) GetLocalPluginList(ctx *gin.Context) {
	list, err := this.eveService.GetLocalPlugins(ctx)
	if err != nil {
		this.Error(ctx, err)
		return
	}
