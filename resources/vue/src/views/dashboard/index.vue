      ...
      <el-row :gutter="10" class="mt-5">
        ...
        <el-col :xs="24" :sm="24" :lg="12">
          <el-card >
            <template #header>
              <div class="flex-x-between">
                <span class="text-[var(--el-text-color-secondary)]">Ev插件数</span>
                <el-tag type="primary">{{articleData.pluginList.length}}</el-tag>
              </div>
            </template>

            <div
              class="flex-x-between mt-2 text-sm text-[var(--el-text-color-secondary)]"
              v-for="(item) in articleData.pluginList"
            >
              <span> {{item.plugin_name}} </span>
              <template v-if="item.has_update">
                <el-tag type="danger">有新版本更新:v{{item.update_version}},本地版本：v{{item.version}}</el-tag>
                <el-button @click="installPlugin(item.plugin_id,item.update_version)" v-loading="installLoading" type="warning">一键升级</el-button>
              </template>
              <template v-else>
                <el-tag type="success">v{{item.version}}</el-tag>
              </template>
            </div>
...
import {GetWxArticleList, GetLocalPluginList, InstallPlugin} from "@/api/plugins";
...
const getLocalPluginList = async ()=>{
  ...
  articleData.pluginList = res.data
