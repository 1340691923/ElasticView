...
func NewNavicatService(esClient pkg.EsI) *NavicatService {
	return &NavicatService{esClient: esClient}
}

func (this *NavicatService) CrudGetList(ctx context.Context, crudFilter *escache.CrudFilter) (res json.RawMessage, count int64, err error) {
	...
	searchBody := Search{
		Query: queryBody,
		From:  int(db.CreatePage(crudFilter.Page, crudFilter.Limit)),
		Size:  crudFilter.Limit,
		Sort:  sortArr,
	}

	resp, err := this.esClient.Search(
		ctx, req, searchBody,
	)
	if err != nil {
		return
...
func (this *NavicatService) CrudGetDSL(ctx context.Context, crudFilter *escache.CrudFilter) (res Search, err error) {
	q, err := GetWhereSql(crudFilter.Relation)
	if err != nil {
		return
	}
...
func (this NavicatService) CrudDownload(ctx context.Context, filter *escache.CrudFilter) (downloadFileName string, titleList []string, searchData [][]string, err error) {

	mappingRes, err := this.esClient.GetMapping(ctx, []string{filter.IndexName})
	if err != nil {
		log.Println("err", err)
		return
	}
	...
	for haveData {
		...
		searchResp, err = this.esClient.Search(ctx, proto2.SearchRequest{Index: []string{filter.IndexName}}, searchBody)
